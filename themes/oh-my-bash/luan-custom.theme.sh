...
function _omb_theme_PROMPT_COMMAND() {
  ...
  local user_host="${_omb_prompt_red}\u${_omb_prompt_normal}@${_omb_prompt_bold_green}\h${_omb_prompt_normal}"
  ...
  PS1="${dtime}${user_host}: ${current_dir} ${python_venv}${ruby_env}${git_branch} $prompt"
}

THEME_CLOCK_COLOR=${THEME_CLOCK_COLOR:-"$_omb_prompt_yellow"}
THEME_CLOCK_FORMAT=${THEME_TIME_FORMAT:-"%H:%M:%S "}
